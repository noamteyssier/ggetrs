...
pub fn python_enrichr<'py>(
    ...
    let results = enrich(add_list.user_list_id, library_name, None).expect("Unable to perform `enrich`");
...
pub fn python_enrichr_background<'py>(
    ...
    let background_list = add_background(&background_list).expect("Unable to query `addBackground`");
    let results = enrich(add_list.user_list_id, library_name, Some(&background_list.backgroundid)).expect("Unable to perform `enrich`");
    results.as_pydict(py)
}
